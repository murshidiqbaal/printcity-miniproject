...
<body>
    ...
    <?php
    // Fetch offer products from the database
    $sql = "SELECT * FROM offer_products";
    $result = mysqli_query($conn, $sql);

    if (!$result) {
        die("Error fetching offer products: " . mysqli_error($conn));
    }

    // Fetch all rows into an array for easy reuse
    $offerProducts = mysqli_fetch_all($result, MYSQLI_ASSOC);
    ?>

    <div class="offer-products-right">
      <?php foreach ($offerProducts as $index => $row): ?>
        <?php if ($index % 2 != 0): // odd index -> right column ?>
          <div class="offer-product-card slide-in-left">
    ...
    <div class="offer-products-left">
      <?php foreach ($offerProducts as $index => $row): ?>
        <?php if ($index % 2 == 0): // even index -> left column ?>
          <div class="offer-product-card slide-in-right">
            <img src="../../Admin/Products/<?= htmlspecialchars($row['image_path']) ?>"
                 alt="<?= htmlspecialchars($row['name']) ?>">
            <h3><?= htmlspecialchars($row['name']) ?></h3>
          </div>
...
window.addEventListener("scroll", () => {
    ...
    if (scrollPos >= triggerPoint) {
        const leftCards = document.querySelectorAll(".offer-products-left .offer-product-card");
        const rightCards = document.querySelectorAll(".offer-products-right .offer-product-card");
